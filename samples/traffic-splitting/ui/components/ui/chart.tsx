...
// Example of a custom tooltip. Adjust the look, styling, and text as needed.
export function ChartTooltipContent({ active, payload, label }: TooltipProps<number, string>) {
  if (!active || !payload || !payload.length) {
    return null
  }
  
  // The first item in payload is typically our bar data.
  const { value } = payload[0]

  return (
    <div className="rounded-lg bg-white px-3 py-1 shadow-md">
      <div className="font-medium">{`Method: ${label}`}</div>
      <div className="text-gray-600">{`Count: ${value}`}</div>
    </div>
  )
}
